...
- Arbitrary `[Npx]` values for spacing **or sizing** when the value sits on the quarter-rem grid — `w-[104px]` must be `w-26` (divide px by 4). Deliberate exceptions are limited to locked chrome metrics (`ui/badge` `text-[12px] leading-[18px]`, `ui/alert` `text-[13px]`/`rounded-[14px]`, email templates); don't extend that list.
