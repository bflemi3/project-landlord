...
function readTerms(locale: Locale): string {
  ...
  const msg = `Missing legal/terms markdown for locale "${locale}", falling back to ${defaultLocale}.`
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: `${origin}${localizedPath(locale, 'terms')}`,
      languages: {
        en: `${MARKETING_ORIGIN.en}${localizedPath('en', 'terms')}`,
        'pt-BR': `${MARKETING_ORIGIN['pt-BR']}${localizedPath('pt-BR', 'terms')}`,
        'x-default': `${MARKETING_ORIGIN.en}${localizedPath('en', 'terms')}`,
      },
    },
  }
}
