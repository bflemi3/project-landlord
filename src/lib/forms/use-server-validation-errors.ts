...
export function useServerValidationErrors<TFields>() {
  const [serverErrors, setServerErrors] = useState<ValidationFieldErrors<TFields>>({})

  const clearServerErrors = useCallback((...fields: Array<keyof TFields & string>) => {
  ...
  const hasFieldError = useCallback(
    (form: Pick<UseFormValidationReturn<unknown>, 'hasError'>, field: keyof TFields & string) =>
      form.hasError(field) || hasServerError(field),
    [hasServerError],
  )

  const getFieldError = useCallback(
    (form: Pick<UseFormValidationReturn<unknown>, 'errors'>, field: keyof TFields & string) =>
