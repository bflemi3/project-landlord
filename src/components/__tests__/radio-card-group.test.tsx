...
describe('RadioCardGroup', () => {
  it('renders all options with labels', () => {
    render(<RadioCardGroup options={options} value={null} onValueChange={() => {}} />)
  ...
  it('marks the selected option as checked', () => {
    render(<RadioCardGroup options={options} value="b" onValueChange={() => {}} />)
  ...
  it('calls onValueChange with the clicked option value', () => {
    ...
    render(<RadioCardGroup options={options} value={null} onValueChange={onChange} />)
  ...
  it('renders a radiogroup with the provided aria-label', () => {
    ...
    expect(screen.getByRole('radiogroup')).toHaveAttribute('aria-label', 'Pick one')
  })
  ...
  it('renders data-slot attributes', () => {
    render(<RadioCardGroup options={options} value="a" onValueChange={() => {}} />)
    expect(screen.getByRole('radiogroup').getAttribute('data-slot')).toBe('radio-card-group')

    const radios = screen.getAllByRole('radio')
    radios.forEach((radio) => expect(radio.getAttribute('data-slot')).toBe('radio-card'))
  })

  it('passes size-6 className to icons', () => {
    render(<RadioCardGroup options={options} value={null} onValueChange={() => {}} />)
    const icons = screen.getAllByTestId('stub-icon')
    icons.forEach((icon) => expect(icon.getAttribute('class')).toContain('size-6'))
  })

  it('renders with a single option', () => {
    const single: RadioCardOption<'only'>[] = [{ value: 'only', label: 'Only', icon: StubIcon }]
    render(<RadioCardGroup options={single} value="only" onValueChange={() => {}} />)
    const radios = screen.getAllByRole('radio')
