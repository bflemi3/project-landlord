...
const inputVariants = cva(
  ...
  // surrounding content.
  'border-input file:text-foreground placeholder:text-muted-foreground focus-visible:border-ring focus-visible:ring-ring/25 disabled:bg-input/50 aria-invalid:border-destructive aria-invalid:ring-destructive/20 dark:disabled:bg-input/80 dark:aria-invalid:border-destructive/50 dark:aria-invalid:ring-destructive/40 h-12 w-full min-w-0 rounded-md border px-4 text-base transition-colors outline-none file:inline-flex file:h-6 file:border-0 file:bg-transparent file:text-sm file:font-medium focus-visible:ring-2 disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50 aria-invalid:ring-2 read-only:cursor-default read-only:border-transparent read-only:focus-visible:border-transparent read-only:focus-visible:ring-0',
  {
    variants: {
...
function Input({
  ...
  const showClear =
    !nonClearableTypes.includes(type ?? 'text') && currentValue.length > 0 && !disabled && !readOnly
  ...
  return (
    <div className="relative min-w-0 flex-1">
      <InputPrimitive
        ...
        disabled={disabled}
        readOnly={readOnly}
        className={cn(inputVariants({ variant }), showClear && 'pr-10', className)}
        {...props}
      />
      {showClear && (
        <button
