...
**Architecture:** Extend existing `providers` and `provider_invoice_profiles` tables. Create `company_cache`, `external_call_log`, and `engineer_allowlist` tables. Define the `BillExtractionResult` contract and `Provider` interface that all subsequent plans depend on.
...
src/lib/billing-intelligence/
  types.ts                              # BillExtractionResult, shared types
  ...
  providers/
    ...
    enliv-campeche/
      ...
      parser.ts                         # bill text → BillExtractionResult
...
export interface BillExtractionResult {
  ...
  rawSource: BillExtractionSource
}

export type BillExtractionSource = 'pdf' | 'api' | 'dda' | 'ocr' | 'web-scrape' | 'email'
...
export interface BillExtractionConfidence {
  fields: Record<string, FieldConfidence>
  source: {
    method: BillExtractionSource
